...
public class IL_GEnhanceNPCs_AI : BaseHook {
    ...
    private void ManipulateIL( ILContext il ) {
        ...
        c.EmitDelegate( () => { return MainConfigCache.优化模式_GEnhanceNPCs_AI == MainConfigs.优化模式.关闭补丁 || !System_补丁自检.未发生已知错误; } );
    ...
    private static void OptimizedCode( NPC npc ) {
        switch ( MainConfigCache.优化模式_GEnhanceNPCs_AI ) {
            ...
            case MainConfigs.优化模式.智能缓存:
                var activeIndices = System.Runtime.InteropServices.CollectionsMarshal.AsSpan( System_缓存_动态数据.生效宠物索引_NPCAI );
                var actions = System_缓存_静态数据.委托映射_BaseEnhance_NPCAI;
