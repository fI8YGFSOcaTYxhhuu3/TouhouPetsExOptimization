...
public class System_缓存_动态数据 : ModSystem {
    ...
    private void 刷新宠物缓存( IList 主动宠物列表, IList 被动宠物列表 ) {
        清空宠物缓存();
        Array.Clear( 宠物索引重复状态, 0, 宠物索引重复状态.Length );
        构筑宠物缓存( 主动宠物列表 );
        构筑宠物缓存( 被动宠物列表 );
    }

    private void 构筑宠物缓存( IList 生效宠物列表 ) {
        ...
        foreach ( object 宠物 in 生效宠物列表 ) {
            ...
            if ( 宠物索引重复状态[ 宠物索引 ] ) continue; else 宠物索引重复状态[ 宠物索引 ] = true;
